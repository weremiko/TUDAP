    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="container mx-auto max-w-5xl px-4 py-12 md:py-16 flex-1">
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-14">
          {([['kurucu', 'Kurucu'], ['akademik_danisman', 'Akademik Danışman']] as const).map(([key, label]) => <section key={key} className="border-t-2 border-primary pt-4"><p className="text-xs uppercase tracking-widest text-accent font-medium">{label}</p><p className="text-sm leading-7 text-muted-foreground mt-3 whitespace-pre-line">{getSection(sections, key)}</p></section>)}
        </div>
